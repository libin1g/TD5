 ...
 *
 *    java和c++类似又有不同:
 *    java和c++相同的是, 非引用类型变量也是唯一代表一个数据存储区,引用类型变量也是唯一代表被引用者所代表的存储区;
 *    无论非引用类型变量还是引用类型变量, 变量本身就是其所代表的数据存储区的起始地址.
 *    不同的是, java引用类型变量所代表的数据存储区的起始地址值在运行期被初始化(缺省初始化为null),整个运行期可以被更改.
 *
